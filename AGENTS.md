...
- `./scripts/verify` runs module tidy checking, tests with coverage report
  (hard-floored at 85%; raise the constant in `scripts/verify` when
  coverage clears the next threshold), vet, race tests, smoke, guardrails,
  release-check, lint, and whitespace checks. This is the canonical
  one-shot. Target is 90; the remaining gap is in defensive SQL-error
  returns and needs a driver wrapper that can inject failures.

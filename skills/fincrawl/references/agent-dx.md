...
| Input hardening | 3 | Provider IDs and archive output paths reject traversal, query fragments, control characters, and encoded separators; provider path parameters are escaped before HTTP dispatch. |
...
- Use `sync --dry-run` and `archive --dry-run` before side-effecting commands.

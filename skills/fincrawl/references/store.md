...
fixture-based encrypted archive output. A DB-backed `publish`/`subscribe`
workflow is planned but not the default contract yet.

Do not invent `publish` or `subscribe` commands until `metadata --json` or
